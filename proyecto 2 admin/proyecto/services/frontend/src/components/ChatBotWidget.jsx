...
export default function ChatBotWidget() {
  ...
  const loadConversation = async (convSessionId) => {
    try {
      console.log('🔵 Cargando conversación:', convSessionId);
      const history = await chatbotAPI.getSessionHistory(convSessionId);
      console.log('🔵 Historial recibido:', history);

      // Manejar respuesta como objeto o como array de pares
      let messages = [];
      if (Array.isArray(history)) {
        // Si es un array de pares [['key', 'value'], ...], convertir a objeto
